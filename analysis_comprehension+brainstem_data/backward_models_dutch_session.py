...
for condition in ['cleanDutch', 'lbDutch', 'mbDutch', 'hbDutch']:
    ...
    y_train = np.hstack(attended_envelopes[:3])[None, :]
    y_test = attended_envelopes[3][None, :]
    ...
    for participant in session_info['dutch_session_participants']:
        ...
        coef = np.linalg.inv(X_train @ X_train.T + 1*np.eye(X_train.shape[0])) @ X_train @ y_train.T
        y_pred = X_test.T @ coef

        print(pearsonr(y_pred.squeeze(), y_test.squeeze())[0])
